...
//        CHConfiguration.shared.apiKey = ""
//        CHConfiguration.shared.clientId = ""
        CHConfiguration.shared.appGroup = "group.candyhouse.widget"
        ...
        CHDeviceManager.shared.getCHDevices(result: {
            if case let .success(devices) = $0 {
                ...
                for device in self.autoUnlockDevices { device.setAutoUnlockFlag(false) }
            }
        })
            ...
            self.autoUnlockTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: true, block: { [unowned self] _ in
                for sesame2 in self.autoUnlockDevices.filter({ $0.autoUnlockFlag() == true }) {
                    ...
                    if sesame2.deviceStatus.loginStatus == .logined && sesame2.deviceStatus != .unlocked() {
                        (sesame2 as? CHSesame5)?.unlock { unlockResult in }
...
//     使用 BackgroundModes 的 remote notifications 功能時，在通知 aps 結構中有 "content-available" = 1 時會呼叫
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter,
        ...
        if let historyTagDic = userInfo["historyTag"] as? [String: Any],
        let historyTag = historyTagDic["data"] as? [UInt8] {
            let historyTagHex = Data(historyTag).toHexString()
            let clientHistoryTagHex = Sesame2Store.shared.getHistoryTag().toHexString()
            if historyTagHex == clientHistoryTagHex {
                completionHandler([])
                return
            }
        }
