...
extension BluetoothSesame2ControlViewController {
    ...
    @IBAction func startTest(_ sender: Any) {
        if (sender as? UIButton)?.isSelected == false {
            ...
            toggle {
                self.toggle {
                    self.startTest(self.startTestButton!)
                }
            }
            
        } else {
            alertIndicator.dismiss(animated: true, completion: {
                self.appendLog(self.sesame!.deviceId.uuidString)
                self.appendLog("----------Test End----------")
                (sender as? UIButton)?.setTitle("Start Test", for: .normal)
                (sender as? UIButton)?.isSelected = false
                
                if self.testErrors.count > 0 {
                    self.completeFailedHandler()
                } else {
                    self.completeSucceedHandler()
                }
            })
        }
    }
    ...
    @IBAction func startLock(_ sender: Any) {
        ...
        @IBAction func readAutolockClick(_ sender: UIButton) {
            ...
            sesame?.getAutolockSetting { result  in
                switch result {
                case .success(let delay):
                    self.autolockLB.text = String(delay.data)
        ...
        @IBAction func dufClick(_ sender: UIButton) {
            do {
                executeOnMainThread {
                    let dfuAlertController = DFUAlertController.instanceWithSesame2(self.sesame!)
                    self.present(dfuAlertController, animated: true, completion: {
                        dfuAlertController.startDFU()
                    })
                }
//
//                let zipData = try Data(contentsOf: Bundle.resourceBundle.url(forResource: nil, withExtension: ".zip")!)
...
//                        }
//
//                        self.dfuHelper = DFUHelper(peripheral: peripheral, zipData: zipData)
...
//                            self.dfuHelper = nil
//                        }
//                        self.dfuHelper?.delegate = progressIndicator
...
extension BluetoothSesame2ControlViewController {
    ...
    private func completeSucceedHandler() {
        ...
        let ok = UIAlertAction(title: "Reset and Exit", style: .default) { _ in
            ViewHelper.showLoadingInView(view: self.view)
            self.sesame!.resetSesame2 { result in
                switch result {
                case .success(_):
                    executeOnMainThread {
                ...
                case .failure(let error):
                    executeOnMainThread {
                        self.appendLog("Reset Sesame2: \(error)")
                    }
                }
            }
        }
        let testAgain = UIAlertAction(title: "Again", style: .default) { _ in
            self.startTest(self.startTestButton!)
        }
