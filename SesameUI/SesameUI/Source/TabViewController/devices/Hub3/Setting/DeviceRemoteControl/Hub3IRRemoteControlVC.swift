...
class Hub3IRRemoteControlVC: CHBaseViewController {
//    private let irRemote:IRRemote
    private let viewModel: Hub3IRRemoteViewModel
    var chDevice: CHHub3!
    private var cooldownIndex: Int? = nil  // 记录正在冷却的 cell 索引
    private var saveButton: UIBarButtonItem!
    private lazy var currentIrRemoteExpandIndex : Int = 0
    ...
    init(irRemote: IRRemote) {
        self.viewModel = Hub3IRRemoteViewModel(remoteRepository: RemoteRepository())
    ...
    private func initParams() {
        currentIrRemoteExpandIndex = chDevice.preference.selectExpandIndex
    }
    ...
    @objc func modifyName(){
        ...
        ChangeValueDialog.showInView(viewModel.getDeviceAlias(),
            ...
            if let index = self?.currentIrRemoteExpandIndex,
               self?.chDevice.irRemotes[safe: index] != nil {
    ...
    private func updateFooterText() {
        viewModel.getIrRemoteType() { [weak self] type in
            guard let self = self else { return }
            executeOnMainThread {
                if type.isEmpty || type.count == 0 {
                    let model:String = self.viewModel.irRemote?.model ?? ""
                    self.footerLabel.text = " \("co.candyhouse.hub3.ir_support_info".localized) \(model)"
                } else {
                    self.footerLabel.text = "\("co.candyhouse.hub3.ir_support_info".localized) \(type)"
                }
            }
        }
    }
    ...
    @objc func quickMatchTapped() {
        ...
        let matchVC = Hub3IRRemoteMatchControlVC.instance(device:viewModel.getDevice(),irReomte: viewModel.irRemote!) {[weak self] successful, deviceInfo in
            guard let self = self else { return }
            if successful {
    ...
    private func updateReomteDeviceInfo() {
        currentIrRemoteExpandIndex = chDevice.preference.selectExpandIndex
        let irRemote = chDevice.irRemotes[currentIrRemoteExpandIndex]
    ...
    private func postIRDevice(_ completion: @escaping (Bool) -> Void) {
        ...
        guard chDevice.irRemotes[safe: currentIrRemoteExpandIndex] == nil else {
            return
        }
        viewModel.addIRDeviceInfo(){[weak self] result in
            ...
            chDevice.preference.updateSelectExpandIndex(chDevice.irRemotes.firstIndex(where: { $0.uuid == self.viewModel.irRemote!.uuid })!)
            currentIrRemoteExpandIndex = chDevice.preference.selectExpandIndex
            executeOnMainThread {
    ...
    private func postDeviceAlias(_ alias: String) {
        ...
        viewModel.modifyRemoteIrDeviceInfo(alias: alias){[weak self] result in
            ...
            if(result) {
                ...
                configureTitle()
                currentIrRemoteExpandIndex = chDevice.preference.selectExpandIndex
                chDevice.irRemotes[currentIrRemoteExpandIndex].updateAlias(alias)
            }
        }
    }
}

extension Hub3IRRemoteControlVC: UICollectionViewDataSource, UICollectionViewDelegate {
...
extension Hub3IRRemoteControlVC {
    static func instance(irRemote:IRRemote) -> Hub3IRRemoteControlVC {
        return Hub3IRRemoteControlVC(irRemote: irRemote)
    }
}
