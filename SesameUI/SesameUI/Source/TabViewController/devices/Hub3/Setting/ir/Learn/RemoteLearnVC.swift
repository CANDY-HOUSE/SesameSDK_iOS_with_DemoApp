...
class RemoteLearnVC: CHBaseViewController, ShareAlertConfigurator {
    
    public private(set) var viewModel: RemoteLearnViewModel!
    ...
    init(viewModel: RemoteLearnViewModel!) {
    ...
    @objc func modifyName(){
        ...
        ChangeValueDialog.showInView(viewModel.remote!.alias,
                                     viewController: self,
    ...
    private func postDeviceAlias(_ alias: String) {
        ...
        viewModel.updateIRDevice(alias) { [weak self] res in
    ...
    func configureEmptyView() {
        ...
        let emptyHit = isPresentedVc ? "co.candyhouse.hub3.ir_empty_controll".localized : "co.candyhouse.hub3.ir_empty_learning".localized
//        let floatView = FloatingTipView.showIn(superView: view, style:  .textOnly(text:emptyHit))
    ...
    func switchMode() {
        viewModel.switchIRMode()
    }
    
    func handleSelectRow(_ ir: IRCode,cell: IRRemoteControlCell) {
        modalSheet(AlertModel(message: nil, sourceView: cell, items: [
            AlertItem(title: "co.candyhouse.hub3.default_ir_name".localized) { _ in
                let name = ir.name == nil || ir.name!.isEmpty ? self.defaultPlaceHolder() : ir.name!
                ChangeValueDialog.show(name, title: "co.candyhouse.hub3.default_ir_name".localized, hint: "co.candyhouse.hub3.irRenameHint".localized) { [unowned self] name in
                    if name == ir.name { return }
                    self.showOverLayLoadingView(view:cell,backgroudeColor: .white)
                    self.viewModel.changeIRCode(id: ir.keyUUID, name: name) { _ in }
                }
            },
            AlertItem(title: "co.candyhouse.sesame2.Delete".localized, style: .destructive) { [unowned self] _ in
                self.showOverLayLoadingView(view: cell,backgroudeColor: .white)
                self.viewModel.deleteIR(id: ir.keyUUID) { _ in }
            },
...
extension RemoteLearnVC: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.funcLabel.text = model.name == nil || model.name!.isEmpty ? defaultPlaceHolder() : model.name
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let model = viewModel.dataSource[indexPath.row]
        self.viewModel.subscribeServerForPostIRDataWhenEmitCode(model.keyUUID)
        viewModel.emitIRCode(id: model.keyUUID) { [self] isSucc in
            if isSucc {
...
extension RemoteLearnVC {
    static func instance(hub3DeviceId: String, remote: IRRemote?) -> RemoteLearnVC {
        let vc =  RemoteLearnVC(viewModel: RemoteLearnViewModel(hub3DeviceId: hub3DeviceId, remote: remote))
        return vc
    }
}
