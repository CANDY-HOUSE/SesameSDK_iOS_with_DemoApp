...
class GPSMapViewController: CHBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        let minimunValue = 2
        let maximumValue = 50
        sliderView = CHUIViewGenerator.slider(defaultValue: Float(circle?.radius ?? defaultRadius) / 10.0, maximumValue: Float(maximumValue), minimumValue: Float(minimunValue), contentWidth: 350, { [unowned self] slider,event in
            if let circle = circle {
                ...
                Sesame2Store.shared.saveLocationForSesame2(self.sesame2, location: CLLocation(latitude: self.circle!.coordinate.latitude, longitude: self.circle!.coordinate.longitude), radius: self.circle!.radius)
                self.sliderView.updateBubble(withValue: self.formatDistanceText(Int(newRadius)))
            }
        })
    ...
    private func formatDistanceText(_ distance: Int) -> String {
        return "co.candyhouse.sesame2.face.distance".localized + " \(distance)m"
    }
...
extension GPSMapViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        if let _ = annotation as? MKUserLocation {
            ...
            confirmButton.image = UIImage(named: "custom-icon")
