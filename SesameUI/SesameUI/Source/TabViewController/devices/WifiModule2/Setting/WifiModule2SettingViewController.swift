...
//            wifiModule2.getCHDevices { _ in }
    ...
    func arrangeSubViews() {
        ...
        addSesameButtonView = CHUIViewGenerator.plain { [unowned self] button,_ in
            if self.wifiModuleDeviceModels.count < 3 && (self.wifiModule2.mechStatus as? CHWifiModule2NetworkStatus)?.isAPWork == true {
                let wifiModule2KeysListViewController = WifiModule2KeysListViewController.instance { device in
    ...
    func confirmDropKey(_ sender: UIButton) {
        let trashKey = UIAlertAction(title: "co.candyhouse.sesame2.TrashTheWifiModule2Key".localized,
            ...
            self.wifiModule2.unregisterNotification()
    ...
    func resetWifiModule2() {
        ViewHelper.showLoadingInView(view: self.view)
            ...
            Sesame2Store.shared.deletePropertyFor(self.wifiModule2)
            self.wifiModule2.unregisterNotification()
            self.wifiModule2.reset { resetResult in
                executeOnMainThread {
                    ViewHelper.hideLoadingView(view: self.view)
                    self.navigationController?.popToRootViewController(animated: false)
                    self.dismissHandler?()
