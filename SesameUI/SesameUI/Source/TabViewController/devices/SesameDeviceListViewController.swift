...
class SesameDeviceListViewController: CHBaseViewController {
    ...
    func configureTable() {
        tableViewProxy = CHTableViewProxy(superView: self.view, selectHandler: { [self] it , indxPath  in
            if it.rawValue is CHDevice {
                ...
                navigationController?.pushViewController(CHWebHostViewController.instanceWithScene("ir-remote",extInfo:extInfo), animated:true)
            }
    ...
    func handleSelectDeviceItem(_ device: CHDevice, _ indexPath: IndexPath) {
        ...
        case .sesame5, .sesame5Pro, .sesame5US, .sesame6, .sesame6Pro, .sesame6ProSlidingDoor, .bleConnector, .sesameMiwa:
        ...
        case .bikeLock2, .bikeLock3:
            ...
            if bikeLockPlus.keyLevel == KeyLevel.guest.rawValue {
                navigateToBike2SettingViewController(bikeLockPlus)
            } else {
