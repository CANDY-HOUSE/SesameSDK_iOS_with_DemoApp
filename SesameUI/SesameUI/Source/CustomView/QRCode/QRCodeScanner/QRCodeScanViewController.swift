...
class QRCodeScanViewController: CHBaseViewController {
    ...
    @objc private func rotatedCamera() {
        if let supportedOrientations = Bundle(for: L.self).object(forInfoDictionaryKey: "UISupportedInterfaceOrientations") as? [String] {
...
extension QRCodeScanViewController: QRScannerViewDelegate {
    ...
    // MARK: playSound
    func playSound() {
        guard let url = Bundle(for: L.self).url(forResource: "bee", withExtension: "mp3") else { return }
        do {
