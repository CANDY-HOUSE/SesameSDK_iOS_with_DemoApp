...
extension URL {
    ...
    static let sesameURLSchemePrefix = "ssm://UI"
    
    static var sesame2UI: URL {
        URL(string: sesameURLSchemePrefix)!
    }
...
//        L.d("keyIndex",deviceKey.keyIndex)
    ...
    func deviceKeyFromQRCodeURL() -> CHDeviceKey? {
        ...
        guard let deviceData = Data(base64Encoded: b64DeviceKey, options: []) else {
            return nil
        }
        ...
        guard let _ = CHProductModel(rawValue: UInt16(type)) else {
            return nil
        }
//        L.d("model",model)
        ...
        // [joi todo] cc check 
        if(model!.is4ByetsPubkeyProductModel){
