...
class OpenSesame2IntentHandler: NSObject, OpenSesame2IntentHandling {
    var completion: ((OpenSesame2IntentResponse) -> Void)!
    var originDelegate: CHSesame2Delegate?
    
    func resolveName(for intent: OpenSesame2Intent, with completion: @escaping (INStringResolutionResult) -> Void) {
    ...
    func handle(intent: OpenSesame2Intent, completion: @escaping (OpenSesame2IntentResponse) -> Void) {
        self.completion = completion
        CHBleManager.shared.enableScan { _ in
            guard let sesame2Name = intent.name else {
                let response = OpenSesame2IntentResponse(code: .failure, userActivity: nil)
                completion(response)
                return
            }
            self.readyToUnlock(sesame2Name)
        }
    }
    
    func readyToUnlock(_ sesame2Name: String) {
...
extension OpenSesame2IntentHandler: CHSesame2Delegate {
    public func onBleDeviceStatusChanged(device: CHSesame2, status: CHSesame2Status,shadowStatus: CHSesame2ShadowStatus?) {
        ...
        if status.loginStatus() == .logined,
            ...
            device.unlock { _ in
                self.unlockCompletedForSesame2(device)
            }
        } else if status.loginStatus() == .logined,
            device.intention == .idle,
            device.mechStatus?.isInUnlockRange == true {
            unlockCompletedForSesame2(device)
        }
    }
    
    func unlockCompletedForSesame2(_ sesame2: CHSesame2) {
        CHDeviceManager.shared.getSesame2s { result in
            switch result {
        ...
        let sesame2Name = Sesame2Store.shared.getSesame2Property(sesame2)?.name ?? sesame2.deviceId.uuidString
        let response = OpenSesame2IntentResponse.success(name: sesame2Name)
        self.completion(response)
        self.completion = nil
    }
